...
export const fetchPokemons = createAsyncThunk(
  'pokemon/fetch',
  async ({ reset = false }: { reset?: boolean }, { getState, rejectWithValue }) => {
    try {
      ...
      const items = await Promise.all(
        list.map(async r => {
          try {
            ...
            if (!id) return null
            const detail = await api.get<PokemonDetailResponse>(`/pokemon/${id}`)
            const image = detail.data.sprites.other?.['official-artwork']?.front_default || null
            const types = detail.data.types.map(t => t.type.name)
            const species = await api.get<PokemonSpeciesResponse>(detail.data.species.url)
            const color = getSpeciesColor(species.data.color?.name)
            const item: PokemonListItem = { id, name: detail.data.name, image, types, color }
            return item
          } catch {
            return null
          }
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<{ items: PokemonListItem[]; next: string | null; reset?: boolean }>) => {
          state.loading = false
          state.refreshing = false
          state.error = null
          state.nextUrl = action.payload.next
          state.items = action.payload.reset ? action.payload.items : [...state.items, ...action.payload.items]
        }
      )
