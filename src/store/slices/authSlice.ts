...
const generateToken = (email: string) =>
  `${Math.random().toString(36).slice(2)}.${Date.now().toString(36)}.${email.replace(/[^a-zA-Z0-9]/g, '')}`
...
export const initAuth = createAsyncThunk('auth/init', async () => {
  ...
  if (Number.isNaN(expNum) || expNum <= now) {
    await Promise.all([removeItem(TOKEN_KEY), removeItem(EXPIRES_KEY), removeItem(EMAIL_KEY)])
...
export const login = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (!isValidEmail(email)) return rejectWithValue('Format email tidak valid')
      if (password.length < 6) return rejectWithValue('Password minimal 6 karakter')
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(initAuth.fulfilled, (state, action: PayloadAction<{ token: string | null; expiresAt: number | null; email: string | null }>) => {
        state.loading = false
      ...
      .addCase(login.fulfilled, (state, action: PayloadAction<{ token: string; expiresAt: number; email: string }>) => {
        state.loading = false
        state.token = action.payload.token
        state.expiresAt = action.payload.expiresAt
        state.email = action.payload.email
